...
### 改了什麼
- 將伺服器每個 tick 傳來的蛇與子彈座標改為前端逐幀插值，畫面不再整格跳動
- 自己的方向輸入會先在本機預測顯示，再以伺服器快照平滑校正；碰撞與計分仍完全由伺服器判定
- 手機跟隨鏡頭改為追蹤插值後的浮點座標，格線與世界會連續捲動
- 伺服器快照加入目前方向，讓前端預測、禁止 180 度轉向及斷線重連保持一致
- 新增移動預測與插值單元測試

### 為什麼改
- 原本 Canvas 只在預設 100ms 的 `game_tick` 到達時重畫，視覺更新率只有約 10 FPS
- 方向鍵只送到伺服器、要等下一次快照才顯示，網路延遲會直接反映在操作手感上
- 跟隨鏡頭使用整數格座標，導致整個畫面每次跳一格後短暫定格

### 影響的檔案
- `frontend/src/components/GameCanvas.jsx` — 逐幀插值、本機預測、平滑跟隨鏡頭
- `frontend/src/gameMotion.js` — 預測與插值的純函式
- `frontend/src/pages/Game.jsx` — 即時方向意圖與伺服器確認
- `backend/src/services/gameService.js`、`backend/src/sockets/roomHandlers.js` — 同步蛇的目前方向
